...
class FinanceTig extends Controller
{
    ...
    {
      $validasi = 0;
      return view('finance/buying/tig/buyingadd')->with('validasi', $validasi)
    ...
    public function tigaddnext_buying($id,$totaluang,$idretur)
    {
      ...
      return view('finance/buying/tig/buying_addnext')->with('buying', $buying)
    ...
    public function tigdo_add_buying(Request $request)
    {
        ...
        if($total_price == 0){
          if($request->validasi==0){
            return redirect(url('finance/buying_home/tig/add/'.$totaluang.'/'.$idretur.'/'.$request->iddetailretur))->with('error', ' Grand Total = 0, Please check Sub Price and Quantities.');
          } else {
            return redirect(url('finance/buying_home/tig/addnext/'.$msid.'/'.$totaluang.'/'.$idretur))->with('error', ' Grand Total = 0, Please check Sub Price and Quantities.');
          }
        }
        // validasi quantities-------------------------------------------------------------------
        ...
        return redirect(url('finance/buying_home/tig/addnext/'.$msid.'/'.$totaluang.'/'.$idretur))->with('status', ' Created new buying has been success.');
    }
    ...
    // manage buying checkout function--------------------------------------------------
		public function tigindex_buying_checkout($id,$totaluang,$idretur)
		{
			...
			return view('finance/buying/tig/checkout')->with('total', $total)
																						->with('buying', $buying)
                                            ->with('idretur', $idretur)
																						->with('totaluang', $totaluang);
		}
		      ...
		      $totalpricereal = 0;
					...
					if($request->status_payment != '1'){
					    if($request->sell_dp > $totalpricereal){
					        return redirect(url('finance/buying_home/tig/checkout'))->with('error', ' Down payment must less than Grand Total');
					    }
					...
					return redirect(url('finance/return_home/addnext/'.$idretur));
				}
		// ---------------------------------------------------------------------------------
    ...
    public function delete_buying_detail($detailid,$buyingid,$totaluang,$idretur)
		{
			...
			return redirect(url('finance/buying_home/tig/addnext/'.$buyingid.'/'.$totaluang.'/'.$idretur))->with('status', ' Deleted buying has been success.');
		}
    ...
    public function delete_selling_detail($detailid,$sellingid,$totaluang,$idretur)
      ...
      manage_selling_history::destroy($detailid);
			return redirect(url('finance/selling_home/tig/addnext/'.$sellingid.'/'.$totaluang.'/'.$idretur))->with('status', ' Deleted buying has been success.');
		}
    ...
    {
        $validasi = 0;
        return view('finance/selling/tig/sellingadd')->with('validasi', $validasi)
                                                  ->with('idretur', $idretur)
                                                  ->with('iddetailretur', $iddetailretur)
                                                  ->with('totaluang', $totaluang);
    }
    ...
    public function tigaddnext_selling($id,$totaluang,$idretur)
    {
      ...
      return view('finance/selling/tig/sellingaddnext')->with('selling', $selling)
                                                  ->with('validasi', $validasi)
                                                  ->with('idretur', $idretur)
                                                  ->with('totaluang', $totaluang)
                                                  ->with('total', $total);
    ...
    public function tigdo_add_selling(Request $request)
    {
      ...
      if($request->items_id!="addnew"){
          if($request->items_qty_view > $request->items_qty_viewasli){
              if($request->validasi==0){
                return redirect(url('finance/selling_home/tig/add/'.$totaluang.'/'.$idretur.'/'.$request->iddetailretur))->with('error', ' Quantities not Enough.');
              } else {
                return redirect(url('finance/selling_home/tig/addnext/'.$request->id_penjualan.'/'.$totaluang.'/'.$idretur))->with('error', ' Quantities not Enough.');
              }
          }
      }
      ...
      if($total_price == 0){
        if($request->validasi==0){
          return redirect(url('finance/selling_home/tig/add/'.$totaluang.'/'.$idretur.'/'.$request->iddetailretur))->with('error', ' Grand Total = 0, Please check Sub Price and Quantities.');
        } else {
          return redirect(url('finance/selling_home/tig/addnext/'.$msid.'/'.$totaluang.'/'.$idretur))->with('error', ' Grand Total = 0, Please check Sub Price and Quantities.');
        }
      }
      // validasi quantities-------------------------------------------------------------------
      ...
      return redirect(url('finance/selling_home/tig/addnext/'.$msid.'/'.$totaluang.'/'.$idretur))->with('status', ' Created new buying has been success.');
    }
    // ---------------------------------------------------------------------------------
    ...
    public function tigindex_selling_checkout($id,$totaluang,$idretur)
    {
      ...
      return view('finance/selling/tig/sellingcheckout')->with('total', $total)
    ...
    public function tigindex_selling_docheckout(Request $request)
    {
      ...
      if($request->status_payment != '1'){
          if($request->sell_dp > $total_price){
              return redirect(url('finance/selling_home/tig/checkout'))->with('error', ' Down payment must less than Grand Total');
          }
      ...
      return redirect(url('finance/salreturn_home/addnext/'.$idretur));
    }
    // ---------------------------------------------------------------------------------
}
