  ...
  <div class="wrapper">
    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        ...
        <ol class="breadcrumb">
          <li>Dashboard</li>
          <li class="active">Selling Home</li>
        </ol>
      </section>
      ...
      <section class="content">
          <div class="row">
            ...
            {{-- search --}}
            <!-- Search FIX -->
              <form class="form" action="{{ url('/selling/selling_home/getrange') }}" method="post">
                {{csrf_field()}}
                <div class="box">
                  <div class="box-body">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                      ...
                      <input class="form-control" type="text" id="cust_id" name="cust_id" pattern=".{3,}" title="3 characters minimum" @if(isset($arraydata[4])) value="{{$arraydata[4]}}" @endif>
                    </div>

                  </div>
                  <div class="box-footer">
                    <div class="pull-right">
                      <a style="width : 90px;" href="{{url('selling/selling_home')}}" type="button" onclick="return confirm('Are you sure want to reset?')" class="btn btn-warning">Reset</a>
            ...
            {{-- tables --}}
              <div class="box box-primary">
                <div class="box-header">
                  <h3 class="box-title">Data Selling</h3>
                </div>

                <div class="box-body table-responsive">

                @if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->c==1)
                  <a href="{{ url('/selling/selling_home/add') }}" type="button" class="btn btn-primary">Add New Selling</a><br><br>
                  @endif
                  <table id="table-home" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td><a style="width:90px;" href="{{url('/selling/selling_home/detail/'.$selling->id)}}">{{ $selling->penjualan_id }}</a></td>
                        ...
                        @if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->u==1)
                          @if($selling->no_nota=="")
                            <td><a style="width:90px;" href="{{ url('/selling/selling_home/addnext/'.$selling->id) }}" type="button" class="btn btn-warning">Continue</a></td>
                          @else
                            <td><a style="width:90px;" href="{{ url('/selling/selling_home/edit/'.$selling->id) }}" type="button" class="btn btn-info">Edit</a></td>
                          @endif
                        @endif
                        @if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->u==1)
                          @if($selling->no_nota=="")
                            @if(Auth::user()->karyawan_id==$selling->modify_user_id)
                              <td><a style="width:90px;" href="{{ url('/selling/selling_home/delete/'.$selling->id) }}" type="button" onclick="return confirm('Are you sure you want to delete this data ?')" class="btn btn-danger">Delete</a></td>
                            @else
                              <td><a style="width:90px;" href="" type="button" onclick="return confirm('Are you sure you want to delete this data ?')" class="btn btn-danger disabled">Delete</a></td>
                            @endif
                          @else
                            <td><a style="width:90px;" href="{{ url('/selling/selling_home/delete/'.$selling->id) }}" type="button" onclick="return confirm('Are you sure you want to delete this data ?')" class="btn btn-danger">Delete</a></td>
                          @endif
                  ...
                  <div class="box-footer">
                    <form class="form" action="{{ url('/selling/selling_home/export') }}" method="post">
                      {{ csrf_field() }}
      ...
      $(function () {
        ...
        var table = $('#table-home').DataTable({
          ...
          buttons: [
              {
                  extend: 'copyHtml5',
                  title: 'TUNAS ABADI 8 | Data Selling',
                  exportOptions: {
                      columns: [ 0, ':visible' ]
                  }
              },
