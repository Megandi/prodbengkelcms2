  ...
  <div class="wrapper">
    ...
    <a href="{{ url('dashboard/home') }}" class="logo">
     ...
     <nav class="navbar navbar-static-top">
        ...
        <ol class="breadcrumb">
          <li>Dashboard</li>
          <li class="active">Solar Type Home</li>
        </ol>
      </section>
      ...
      <section class="content">
          <div class="row">
            ...
            {{-- search --}}
            <form class="form" action="{{ url('/operational/solar_type_home/getrange') }}" method="post">
              {{-- set token --}}
              {{ csrf_field() }}
              ...
              <div class="box">
                <div class="box-body">
                  ...
                  <div class="col-md-6">
                    ...
                    <label class="control-label">Price Liter</label>
                    <input class="form-control" type="number" id="price" name="price" pattern=".{3,}" title="3 characters minimum" @if(isset($arraydate[3])) value="{{$arraydate[3]}}" @endif>
                ...
                <div class="box-footer">
                  <div class="pull-right">
                    <a style="width : 90px;" href="{{url('operational/solar_type_home')}}" type="button" onclick="return confirm('Are you sure want to reset?')" class="btn btn-warning">Reset</a>
            ...
            {{-- tables --}}
              <div class="box box-primary">
                <div class="box-header">
                  <h3 class="box-title">Data Solar Type</h3>
                </div>

                <div class="box-body table-responsive">
                
                @if(DB::table('level_akses')->where('id_menu', '2')->where('id_level',auth::user()->level_id)->first()->c==1)
                  <a href="{{ url('/operational/solar_type_home/add') }}" type="button" class="btn btn-primary">Add New Solar Type</a><br><br>
                  @endif
                ...
                @if(DB::table('level_akses')->where('id_menu', '2')->where('id_level',auth::user()->level_id)->first()->d==1)
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{{ $item->name }}</td>
                        <td>{{ number_format($item->harga_liter) }}</td>
                ...
                @if(DB::table('level_akses')->where('id_menu', '2')->where('id_level',auth::user()->level_id)->first()->u==1)
                        <td><a style="width:90px;" href="{{ url('/operational/solar_type_home/edit/'.$item->id) }}" type="button" class="btn btn-info">Edit</a></td>
                        @endif

                @if(DB::table('level_akses')->where('id_menu', '2')->where('id_level',auth::user()->level_id)->first()->d==1)
                        <td><a style="width:90px;" href="{{ url('/operational/solar_type_home/delete/'.$item->id) }}" type="button" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this data ?')">Delete</a></td>
                        @endif
                      </tr>
                      @endforeach
                ...
                @if(DB::table('level_akses')->where('id_menu', '1')->where('id_level',auth::user()->level_id)->first()->e==1)
                  <div class="box-footer">
                     <form class="form" action="{{ url('/operational/solar_type_home/export') }}" method="POST">
                        ...
                        <input type="hidden" id="price" name="price" value="<?php if(isset($arraydate[3])){ echo $arraydate[3]; }?>">
      ...
      $(function () {
        ...
        var table = $('#table-home').DataTable({
          ...
          buttons: [
              {
                  extend: 'copyHtml5',
                  title: 'TUNAS ABADI 8 | Data Solar Type',
                  exportOptions: {
                      columns: [ 0, ':visible' ]
                  }
              },
