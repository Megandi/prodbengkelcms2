  ...
  <div class="wrapper">
    ...
    <a href="{{ url('dashboard/home') }}" class="logo">
     ...
     <nav class="navbar navbar-static-top">
        ...
        <ol class="breadcrumb">
          <li>Dashboard</li>
          <li class="active">Service Home</li>
        </ol>
      </section>
      ...
      <section class="content">
          <div class="row">
            ...
            {{-- search --}}
            <form class="form" action="{{ url('/operational/service_home/getrange') }}" method="post">
              {{-- set token --}}
              {{ csrf_field() }}
              ...
              <div class="box">
                <div class="box-body">
                  ...
                  <div class="col-md-6">
                    ...
                    <label class="control-label">Name Service</label>
                  ...
                  <div class="col-md-6">
                    ...
                    <input class="form-control" type="text" id="idservice" name="idservice" pattern=".{3,}" title="3 characters minimum" @if(isset($arraydate[4])) value="{{$arraydate[4]}}" @endif>

                    <label class="control-label">Items Name</label>
                    <input class="form-control name" type="text" id="nameitems" name="nameitems" pattern=".{3,}" title="3 characters minimum" @if(isset($arraydate[5])) value="{{$arraydate[5]}}" @endif>
                ...
                <div class="box-footer">
                  <div class="pull-right">
                    <a style="width : 90px;" href="{{url('operational/service_home')}}" type="button" onclick="return confirm('Are you sure want to reset?')" class="btn btn-warning">Reset</a>
            ...
            {{-- tables --}}
              <div class="box box-primary">
                ...
                @if(DB::table('level_akses')->where('id_menu', '2')->where('id_level',auth::user()->level_id)->first()->c==1)
                  <a href="{{ url('/operational/service_home/add') }}" type="button" class="btn btn-primary">Add New Service</a><br><br>
                  @endif
                ...
                @if(DB::table('level_akses')->where('id_menu', '2')->where('id_level',auth::user()->level_id)->first()->u==1)
                        <td><a style="width:90px;" href="{{ url('/operational/service_home/edit/'.$item->id.'/'.$item->barang_id) }}" type="button" class="btn btn-info">Edit</a></td>
                        @endif

                @if(DB::table('level_akses')->where('id_menu', '2')->where('id_level',auth::user()->level_id)->first()->d==1)
                        <td><a style="width:90px;" href="{{ url('/operational/service_home/delete/'.$item->id) }}" type="button" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this data ?')">Delete</a></td>
                        @endif
                      </tr>
                      @endforeach
                ...
                @if(DB::table('level_akses')->where('id_menu', '1')->where('id_level',auth::user()->level_id)->first()->e==1)
                  <div class="box-footer">
                     <form class="form" action="{{ url('operational/service_home/export') }}" method="POST">
                        ...
                        <input type="hidden" id="idservice" name="idservice" value="<?php if(isset($arraydate[4])){ echo $arraydate[4]; }?>">
                        <input type="hidden" id="nameitems" name="nameitems" value="<?php if(isset($arraydate[5])){ echo $arraydate[5]; }?>">
      ...
      $(function () {
        ...
        var table = $('#table-home').DataTable({
          ...
          buttons: [
              {
                  extend: 'copyHtml5',
                  title: 'TUNAS ABADI 8 | Data Service',
                  exportOptions: {
                      columns: [ 0, ':visible' ]
                  }
              },
