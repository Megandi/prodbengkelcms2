          ...
          <div class="row">
            <div class="col-xs-12">
              <div class="box box-info">
                ...
                <form class="form-horizontal" action="{{ url('/operational/solar_home/do_add') }}" method="post">
                  <div class="box-body">
                    ...
                    <div class="form-group">
                      <label for="input_car_id" class="col-sm-2 control-label">ID Car <span style="color:red;">*</span></label>
                      <div class="col-sm-10">
                        <select style="text-transform:uppercase" class="js-car-id form-control">
                          <option selected>{{old('car_id')}} - {{old('own_id')}}</option>
                        </select>
                       @if($errors->has('car_id'))
                          <p style="font-style: bold; color: red;">{{ $errors->first('car_id') }}</p>
                      @endif
                      <input type="hidden" id="car_id" name="car_id" value="{{old('car_id')}}">
                      <input type="hidden" id="own_id" name="own_id" value="{{old('own_id')}}">
                      </div>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <select style="text-transform:uppercase" class="js-emp-id form-control">
                          <option selected>{{old('emp_id')}} - {{old('emp_name')}}</option>
                        </select>
                       @if($errors->has('emp_id'))
                          <p style="font-style: bold; color: red;">{{ $errors->first('emp_id') }}</p>
                      @endif
                      <input type="hidden" id="emp_id" name="emp_id" value="{{old('emp_id')}}">
                      <input type="hidden" id="emp_name" name="emp_name" value="{{old('emp_name')}}">
                      </div>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <input type="text" class="form-control" name="charger" id="charger" placeholder="Solar Charger" value="{{old('charger')}}">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <div class='input-group date' id='datetimepicker'><input type="text" class="form-control for_date" name="solar_date" id="solar_date"></div>
                        @if($errors->has('solar_date'))
                          <p style="font-style: bold; color: red;">{{ $errors->first('solar_date') }}</p>
                        @endif
                      </div>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <input type="text" class="form-control" name="no_nota" id="no_nota" placeholder="Nota" value="{{old('no_nota')}}">
                      ...
                      <div class="col-sm-10">
                        ...
                        <option value="">Choose Type</option>
                          @foreach($lt_solar as $item)
                            <option @if($item->id == old('solar_type')) selected @endif value="{{$item->id}}">{{$item->name}}</option>
                          @endforeach
                        </select>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <input type="text" class="form-control" name="liter" id="liter" placeholder="Used ( Liter )" value="{{old('liter')}}">
                      @if($errors->has('liter'))
  ...
  <script>
    ...
    function close_window() {
      if (confirm("Are you sure you want to close this page? Any changes you made will not be saved.")) {
        window.location.replace("{{ url('/operational/solar_home') }}");
      }
    }

    $(".js-car-id").select2({
    ajax: {
      url: "{{ url('/operational/solar_home/search_car') }}",
      dataType: 'json',
      delay: 250,
    ...
    $(".js-emp-id").select2({
    ajax: {
      url: "{{ url('/operational/solar_home/search_employee_solar') }}",
      dataType: 'json',
      delay: 250,
      data: function (params) {
        return {
