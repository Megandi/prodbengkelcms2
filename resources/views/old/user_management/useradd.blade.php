  ...
  <div class="wrapper">
          ...
          <div class="row">
            <div class="col-xs-12">
              ...
              <div class="box box-info">
                ...
                <form class="form-horizontal" action="{{ url('/usermanagement/user_home/do_add') }}" method="post">
                  <div class="box-body">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <input type="email" class="form-control" name="email" id="email" placeholder="Email" value="{{old('email')}}" pattern="[a-z0-9._+-]+@[a-z0-9.-]+\.[a-z]{2,3}$">
                      @if($errors->has('email'))
                      ...
                      <div class="col-sm-10">
                        ...
                        <option value="">Choose Department</option>
                          @foreach($ms_department as $item)
                            <option @if($item->id == old('department_id')) selected @endif value="{{$item->id}}">{{$item->name}}</option>
                          @endforeach
                        </select>
                      ...
                      <div class="col-sm-10">
                        ...
                        <option value="">Choose position</option>
                          @foreach($ms_jabatan as $item)
                            <option @if($item->id == old('position_id')) selected @endif value="{{$item->id}}">{{$item->name}}</option>
                          @endforeach
                        </select>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        ...
                        <option value="">Choose Level</option>
                          @foreach($lt_user_type as $item)
                            <option @if($item->id == old('level_id')) selected @endif value="{{$item->id}}">{{$item->name}}</option>
                          @endforeach
                        </select>
                     ...
                     <div class="form-group">
                      <label for="input_password" class="col-sm-2 control-label">Re Password <span style="color:red;">*</span></label>
                      <div class="col-sm-10">
    ...
    function close_window() {
      if (confirm("Are you sure you want to close this page? Any changes you made will not be saved.")) {
        window.location.replace("{{ url('/usermanagement/user_home') }}");
      }
    }

		$(".js-karyawan-id").select2({
		ajax: {
			url: "{{ url('/usermanagement/user_home/search_employee') }}",
			dataType: 'json',
			delay: 250,
