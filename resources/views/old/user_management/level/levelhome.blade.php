            ...
            {{-- tables --}}
              <div class="box box-primary">
                ...
                @if(DB::table('level_akses')->where('id_menu', '4')->where('id_level',auth::user()->level_id)->first()->c==1)
                  <a href="{{ url('/usermanagement/level_home/add') }}" type="button" class="btn btn-primary">Add New Level</a><br><br>
                  @endif
                ...
                @if(DB::table('level_akses')->where('id_menu', '4')->where('id_level',auth::user()->level_id)->first()->d==1)
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td><a style="width:90px;" href="{{ url('/usermanagement/level_home/set_menu/'.$item->id) }}" type="button" class="btn btn-warning">Set Menu</a></td>
                 
                @if(DB::table('level_akses')->where('id_menu', '4')->where('id_level',auth::user()->level_id)->first()->u==1)
                        <td><a style="width:90px;" href="{{ url('/usermanagement/level_home/edit/'.$item->id) }}" type="button" class="btn btn-info">Edit</a></td>
                        @endif

                @if(DB::table('level_akses')->where('id_menu', '4')->where('id_level',auth::user()->level_id)->first()->d==1)
                        <td><a style="width:90px;" href="{{ url('/usermanagement/level_home/delete/'.$item->id) }}" type="button" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this data ?')">Delete</a></td>
                        @endif
                      </tr>
                      @endforeach
