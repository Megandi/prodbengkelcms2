    ...
    <div class="content-wrapper">
      ...
      <section class="content">
          <div class="row">
            ...
            <!-- Search FIX -->
              <form class="form" action="{{ url('/finance/debt_home/getrange') }}" method="post">
                {{csrf_field()}}
                <div class="box">
                  <div class="box-body">
                    <div class="col-md-6">
                      ...
                      <label class="control-label">Debit ID</label>
                      <input class="form-control" type="text" id="iddebt" name="iddebt" pattern=".{3,}" title="3 characters minimum" @if(isset($arraydata[3])) value="{{$arraydata[3]}}" @endif>
                    </div>

                    <div class="col-md-6">
                      <label class="control-label">Buying ID</label>
                      <input class="form-control" type="text" id="idbuying" name="idbuying" pattern=".{3,}" title="3 characters minimum" @if(isset($arraydata[2])) value="{{$arraydata[2]}}" @endif>
                  ...
                  <div class="box-footer">
                    <div class="pull-right">
                      <a style="width : 90px;" href="{{url('finance/debt_home')}}" type="button" onclick="return confirm('Are you sure want to reset?')" class="btn btn-warning">Reset</a>
            ...
            {{-- tables --}}
              <div class="box box-primary">
                <div class="box-header">
                  <h3 class="box-title">Data Debt</h3>
                </div>

                <div class="box-body table-responsive">
                  {{-- <a href="{{ url('/finance/debt_home/add') }}" type="button" class="btn btn-primary">Add New Houling</a><br><br> --}}
                  <table id="table-home" class="table table-bordered table-hover">
                    <thead>
                      <tr>
                        ...
                        <th>Date Payment</th>
                        <th>Grand Total</th>
                        <th>Debt already paid</th>
                        <th>Total Debt payable</th>
                        <th>Status</th>
                        <th>Modify by</th>
                        <th>Last Modified</th>
                        <th>Action</th>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>@if($item->status_hutang > 0) {{ 'UNPAID' }} @else {{ 'PAID' }} @endif</td>
                        <td>{{ $item->modify_user_id }}</td>
                        <td><?= date("d/m/Y H:i:s",strtotime($item->last_modify_date));?></td>
                        @if($item->status_hutang > 0)
                          <td><a style="width:90px;" href="{{ url('/finance/debt_home/edit/'.$item->id) }}" type="button" class="btn btn-info">Edit</a></td>
                        @else
                  ...
                  <div class="box-footer">
                    <form class="form" action="{{ url('/finance/debt_home/export') }}" method="post">
                      {{ csrf_field() }}
                      ...
                      <input class="form-control" type="hidden" id="iddebt" name="iddebt" pattern=".{3,}" title="3 characters minimum" @if(isset($arraydata[3])) value="{{$arraydata[3]}}" @endif>
                      ...
                      <input class="form-control" type="hidden" id="idbuying" name="idbuying" pattern=".{3,}" title="3 characters minimum" @if(isset($arraydata[2])) value="{{$arraydata[2]}}" @endif>
