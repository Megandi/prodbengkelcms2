...
<body class="hold-transition skin-blue sidebar-mini">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        @if($detail->type_return == 4 )
                          ...
                          @if(!$penjualan)
                            <!-- add  -->
                            <td><a style="width:90px;" href="{{url('finance/selling_home/tig/add/'.$detail->qty*$detail->sub_total.'/'.$return->id.'/'.$detail->id)}}" type="button" class="btn btn-warning">Continue</a></td>
                          @elseif($penjualan->no_nota=="")
                            <!-- add next -->
                            <td><a style="width:90px;" href="{{url('finance/selling_home/tig/addnext/'.$penjualan->id.'/'.$detail->qty*$detail->sub_total.'/'.$return->id)}}" type="button" class="btn btn-warning">Continue</a></td>
                          @else
                        ...
                        @elseif($detail->type_return == 1 )
                          ...
                          @if(!$credit)
                            <!-- add  -->
                            <td><a style="width:90px;" href="{{url('finance/salreturn_home/addnext/cdb/'.$detail->id.'/'.$return->id.'/'.$detail->qty*$detail->sub_total)}}" type="button" class="btn btn-warning">Continue</a></td>
                          @else
                            <td><a style="width:90px;" href="#" type="button" class="btn btn-default disabled">N/A</a></td>
                          @endif
                        @else
  ...
  <div class="modal fade" id="rejectModalbarang" tabindex="-1" role="dialog" aria-labelledby="rejectModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form class="form-horizontal" action="{{ url('/finance/salreturn_home/do_addnext') }}" method="post">
              {{-- set token --}}
              {{ csrf_field() }}
  ...
  <script>
    ...
    function barang(id){
      ...
      $.ajax({
            url: "{{url('finance/salreturn_home/addnext/getbarang')}}" + "/" + id,
            data: {},
            dataType: "json",
            type: "get",
