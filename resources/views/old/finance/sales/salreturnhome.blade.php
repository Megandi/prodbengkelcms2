    ...
    <div class="content-wrapper">
      ...
      <section class="content">
          <div class="row">
            ...
            {{-- search --}}
              <form class="form" action="{{ url('/finance/salreturn_home/getrange') }}" method="post">
                {{csrf_field()}}
                <div class="box">
                  <div class="box-body">
                    <div class="col-md-6">
                  ...
                  <div class="box-footer">
                    <div class="pull-right">
                      <a style="width : 90px;" href="{{url('finance/salreturn_home')}}" type="button" onclick="return confirm('Are you sure want to reset?')" class="btn btn-warning">Reset</a>
            ...
            {{-- tables --}}
              <div class="box box-primary">
                ...
                @if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->c==1)
                  <a href="{{ url('/finance/salreturn_home/add') }}" type="button" class="btn btn-primary">Created New Return Items</a><br><br>
                  @endif
                  <table id="table-home" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        {{--@if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->u==1)
                        <td><a style="width:90px;" href="{{ url('/finance/saleturn_home/edit/'.$item->id) }}" type="button" class="btn btn-info">Edit</a></td>
                        @endif--}}
                        @if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->d==1)
                        {{--<td><a style="width:90px;" href="{{ url('/finance/salreturn_home/delete/'.$item->id.'/'.$item->stock_id.'/'.$item->total_qty_return.'/'.$item->stock_sell) }}" type="button" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this data ?')">Delete</a></td>--}}
                        <td><a style="width:90px;" href="{{ url('/finance/salreturn_home/addnext/'.$item->id) }}" type="button" class="btn btn-info">Edit</a></td>
                        <td><a style="width:90px;" href="{{ url('/finance/salreturn_home/delete/'.$item->id) }}" type="button" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this data ?')">Delete</a></td>
                        @endif
                      </tr>
                      @endforeach
                  ...
                  <div class="box-footer">
                    <form class="form" action="{{ url('/finance/salreturn_home/export') }}" method="post">
                      {{ csrf_field() }}
