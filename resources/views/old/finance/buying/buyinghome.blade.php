    ...
    <div class="content-wrapper">
      ...
      <section class="content">
          <div class="row">
            ...
            {{-- search --}}
            <!-- Search FIX -->
              <form class="form" action="{{ url('/finance/buying_home/getrange') }}" method="post">
                {{csrf_field()}}
                <div class="box">
                  <div class="box-body">
                    <div class="col-md-6">
                  ...
                  <div class="box-footer">
                    <div class="pull-right">
                      <a style="width : 90px;" href="{{url('finance/buying_home')}}" type="button" onclick="return confirm('Are you sure want to reset?')" class="btn btn-warning">Reset</a>
            ...
            {{-- tables --}}
              <div class="box box-primary">
                ...
                @if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->c==1)
                  <a href="{{ url('/finance/buying_home/add') }}" type="button" class="btn btn-primary">Add New Buying</a><br><br>
                  @endif
                  <table id="table-home" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        @if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->u==1)
                          @if($buying->no_nota=="")
                            <td><a style="width:90px;" href="{{ url('/finance/buying_home/addnext/'.$buying->id) }}" type="button" class="btn btn-warning">Continue</a></td>
                          @else
                            <td><a style="width:90px;" href="{{ url('/finance/buying_home/edit/'.$buying->id) }}" type="button" class="btn btn-info">Edit</a></td>
                          @endif
                        @endif
                        @if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->u==1)
                          @if($buying->no_nota=="")
                            @if(Auth::user()->karyawan_id==$buying->modify_user_id)
                              <td><a style="width:90px;" href="{{ url('/finance/buying_home/delete/'.$buying->id) }}" type="button" onclick="return confirm('Are you sure you want to delete this data ?')" class="btn btn-danger">Delete</a></td>
                            @else
                              <td><a style="width:90px;" href="" type="button" class="btn btn-danger disabled">Delete</a></td>
                            @endif
                          @else
                            <td><a style="width:90px;" href="{{ url('/finance/buying_home/delete/'.$buying->id) }}" onclick="return confirm('Are you sure you want to delete this data ?')" type="button" class="btn btn-danger">Delete</a></td>
                          @endif
                        @endif
                      </tr>
                      @endforeach
                  ...
                  <div class="box-footer">
                    <form class="form" action="{{ url('/finance/buying_home/export') }}" method="post">
                      {{ csrf_field() }}
