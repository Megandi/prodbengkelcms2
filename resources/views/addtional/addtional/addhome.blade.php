     ...
     <nav class="navbar navbar-static-top">
      ...
      <section class="content">
          <div class="row">
            ...
            <!-- Search FIX -->
              <form class="form" action="{{ url('/addtional/addtional_home/getrange') }}" method="post">
                {{csrf_field()}}
                <div class="box">
                  <div class="box-body">
                    <div class="col-md-6">
                  ...
                  <div class="box-footer">
                    <div class="pull-right">
                      <a style="width : 90px;" href="{{url('addtional/addtional_home')}}" type="button" onclick="return confirm('Are you sure want to reset?')" class="btn btn-warning">Reset</a>
            ...
            {{-- tables --}}
              <div class="box box-primary">
                ...
                @if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->c==1)
                  <a href="{{ url('/addtional/addtional_home/add') }}" type="button" class="btn btn-primary">Add New Addtional Cost</a><br><br>
                  @endif
                  <table id="table-home" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        @if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->u==1)
                          @if($item->no_nota=="")
                            <td><a style="width:90px;" href="{{ url('/addtional/addtional_home/addnext/'.$item->id) }}" type="button" class="btn btn-warning">Continue</a></td>
                          @else
                            <td><a style="width:90px;" href="{{ url('/addtional/addtional_home/edit/'.$item->id) }}" type="button" class="btn btn-info">Edit</a></td>
                          @endif
                        @endif
                        @if(DB::table('level_akses')->where('id_menu', '3')->where('id_level',auth::user()->level_id)->first()->u==1)
                          @if($item->no_nota=="")
                            @if(Auth::user()->karyawan_id==$item->modify_user_id)
                              <td><a style="width:90px;" href="{{ url('/addtional/addtional_home/delete/'.$item->id) }}" type="button" class="btn btn-danger">Delete</a></td>
                            @else
                  ...
                  <div class="box-footer">
                    <form class="form" action="{{ url('/addtional/addtional_home/exportt') }}" method="post">
                      {{ csrf_field() }}
