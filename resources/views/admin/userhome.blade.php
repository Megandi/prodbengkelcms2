     ...
     <nav class="navbar navbar-static-top">
      ...
      <section class="content">
          <div class="row">
            ...
            {{-- search --}}
            <div class="row">
              <form class="form" action="{{ url('/admin/user_home/getrange') }}" method="post">
                <div class="box-body">
                {{-- set token --}}
                {{ csrf_field() }}
                  ...
                  <div class="pull-right">
                    <a style="width:90px;" href="{{ url('/admin/user_home') }}" type="button" class="btn btn-warning">Reset</a>
            ...
            {{-- tables --}}
              <div class="box box-primary">
                ...
                @if(DB::table('level_akses')->where('id_menu', '4')->where('id_level',auth::user()->level_id)->first()->c==1)
                  <a href="{{ url('/admin/user_home/add') }}" type="button" class="btn btn-primary">Add New User</a><br><br>
                  @endif
                ...
                @if(DB::table('level_akses')->where('id_menu', '4')->where('id_level',auth::user()->level_id)->first()->u==1)
                        <td><a style="width:90px;" href="{{ url('/admin/user_home/edit/'.$item->id) }}" type="button" class="btn btn-info">Edit</a></td>
                        @endif

                @if(DB::table('level_akses')->where('id_menu', '4')->where('id_level',auth::user()->level_id)->first()->d==1)
                        <td><a style="width:90px;" href="{{ url('/admin/user_home/delete/'.$item->id) }}" type="button" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this data ?')">Delete</a></td>
                        @endif
                      </tr>
                      @endforeach
