...
<body class="hold-transition skin-blue sidebar-mini">
          ...
          <div class="row">
            ...
            @endif
              ...
              <div class="box box-info">
                ...
                <form class="form-horizontal" action="{{ url('/purchase/return_home/do_add') }}" method="post">
                  <div class="box-body">
                    ...
                    <div class="form-group">
                      <label for="input_return_id" class="col-sm-2 control-label">ID Purchase Return</label>
                      <div class="col-sm-10">
                        <input type="text" class="form-control" name="return_id" id="return_id" placeholder="ID Purchase Return" value="{{$return->retur_id}}" readonly="true">
                    ...
                    <div class="form-group">
                      <label for="input_return_id" class="col-sm-2 control-label">ID Buying</label>
                      <div class="col-sm-10">
                        <input type="text" class="form-control" name="return_id" id="return_id" placeholder="ID Purchase Return" value="{{$return->pembelian->pembelian_id}}" readonly="true">
                      @if($errors->has('return_id'))
                          <p style="font-style: bold; color: red;">{{ $errors->first('return_id') }}</p>
                      @endif
                      </div>
                    ...
                    <div class="form-group">
                      <label for="input_return_id" class="col-sm-2 control-label">Supplier</label>
                      <div class="col-sm-10">
                        <input type="text" class="form-control" value="{{$return->pembelian->supplier_id}} - {{$return->pembelian->supplier->nama}}" readonly="true">
                      @if($errors->has('return_id'))
                          <p style="font-style: bold; color: red;">{{ $errors->first('return_id') }}</p>
                      @endif
                      </div>
              ...
              <div class="box box-primary">
                <div class="box-header">
                  List Items in Buying : {{$return->pembelian->pembelian_id}}
                </div>
                  ...
                  <table id="table-home" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{{number_format($detail->qty*$detail->sub_total_pembelian)}}</td>
              ...
              <div class="box box-primary">
                <div class="box-header">
                  List Items in Purchase Return : {{$return->retur_id}}
                </div>
                  ...
                  <table id="table-home1" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <?php $pembelian = App\Models\manage_buying::where('returdetail_id', $detail->id)->first();?>
                        @if($detail->type_return == 4 )
                          @if(!$pembelian)
                            <!-- add  -->
                            <td><a style="width:90px;" href="{{url('buying/buying_home/tig/add/'.$detail->qty*$detail->sub_total.'/'.$return->id.'/'.$detail->id)}}" type="button" class="btn btn-warning">Continue</a></td>
                          @elseif($pembelian->no_nota=="")
                            <!-- add next -->
                            <td><a style="width:90px;" href="{{url('buying/buying_home/tig/addnext/'.$pembelian->id.'/'.$detail->qty*$detail->sub_total.'/'.$return->id)}}" type="button" class="btn btn-warning">Continue</a></td>
                          @else
                            <td><a style="width:90px;" href="#" type="button" class="btn btn-default disabled">N/A</a></td>
                          @endif
                        @elseif($detail->type_return == 1 )
                          <?php $debit = App\Models\manage_debt_history::where('returndetail_id', $detail->id)->first();?>
                          @if(!$debit)
                            <!-- add  -->
                            <td><a style="width:90px;" href="{{url('purchase/return_home/addnext/cdb/'.$detail->id.'/'.$return->id.'/'.$detail->qty*$detail->sub_total)}}" type="button" class="btn btn-warning">Continue</a></td>
                          @else
                            <td><a style="width:90px;" href="#" type="button" class="btn btn-default disabled">N/A</a></td>
                          @endif
                        @else
  ...
  <div class="modal fade" id="rejectModalbarang" tabindex="-1" role="dialog" aria-labelledby="rejectModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form class="form-horizontal" action="{{ url('/purchase/return_home/do_addnext') }}" method="post">
              {{-- set token --}}
              {{ csrf_field() }}
              ...
              <input type="hidden" id="detailpembelianid" name="detailpembelianid">
                      ...
                      <table id="table-home" class="table table-bordered table-hover">
                        <thead>
                          ...
                          <tr>
                            <th>Type Return</th>
                            <th>
                                <select class="form-control" required  name="type_return" id="type_return">
            ...
            <div class="modal-footer">
                <button style="width:90px;" type="submit" onclick="return confirm('Are you sure you want to save this data ?')" class="btn btn-info pull-right">Submit</button>
            </div>
            </form>
	        </div>
        </div>
  ...
  <script>
    ...
    function barang(id){
      ...
      $.ajax({
            url: "{{url('purchase/return_home/addnext/getbarang')}}" + "/" + id,
            data: {},
            dataType: "json",
            type: "get",
            success:function(data)
            {
              ...
              subtotalrp = subtotal.toFixed(2).replace(/./g, function(c, i, a) {
                return i && c !== "." && ((a.length - i) % 3 === 0) ? ',' + c : c;
                    ...
                    $('#detailpembelianid').val(detailpembelianid);
            }
    ...
    function close_window() {
      if (confirm("Are you sure you want to close this page? Any changes you made will not be saved.")) {
        window.location.replace("{{ url('/purchase/return_home') }}");
      }
    }
    ...
    ajax: {
      url: "{{ url('/purchase/return_home/search_buying') }}",
      dataType: 'json',
      delay: 250,
      ...
      var table = $('#table-home').DataTable({
        ...
        // dom: 'lrtipB',
          ...
          buttons: [
              {
                  extend: 'copyHtml5',
                  title: 'TUNAS ABADI 8 | List Items in Buying',
                  exportOptions: {
                      columns: [ 0, ':visible' ]
                  }
              },
      ...
      var table = $('#table-home1').DataTable({
        ...
        // dom: 'lrtipB',
          ...
          buttons: [
              {
                  extend: 'copyHtml5',
                  title: 'TUNAS ABADI 8 | List Items in Purchase Return',
                  exportOptions: {
                      columns: [ 0, ':visible' ]
                  }
              },
