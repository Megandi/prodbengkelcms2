          ...
          <div class="row">
            <div class="col-xs-12">
              <div class="box box-info">
                ...
                <form class="form-horizontal" action="{{ url('/houling/houling_home/do_add') }}" method="post">
                  <div class="box-body">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <select style="text-transform:uppercase" class="js-employee-id form-control">
                          <option selected>{{old('employee_id')}} - {{old('employee_name')}}</option>
                        </select>
                      ...
                      <input type="hidden" id="employee_name" name="employee_name" value="{{old('employee_name')}}">
                      </div>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <select style="text-transform:uppercase" class="js-route-id form-control">
                          <option selected>{{old('route_id')}} {{old('route_a')}} - {{old('route_b')}}</option>
                        </select>
                      ...
                      <div class="col-sm-10">
                        ...
                        <option value="">Choose Type</option>
                          @foreach($lt_solar as $item)
                            <option @if($item->id == old('solar_type')) selected @endif value="{{$item->id}}">{{$item->name .' - '. $item->id}}</option>
                          @endforeach
                        </select>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <select style="text-transform:uppercase" class="js-tonase-id form-control">
                          <option selected>{{old('tonase_id')}}</option>
                        </select>
                       @if($errors->has('tonase_id'))
                          <p style="font-style: bold; color: red;">{{ $errors->first('tonase_id') }}</p>
                      @endif
                      <input type="hidden" id="tonase_id" name="tonase_id" value="{{old('tonase_id')}}">
                      </div>
                    </div>
  ...
  <script>
    ...
    function close_window() {
      if (confirm("Are you sure you want to close this page? Any changes you made will not be saved.")) {
        window.location.replace("{{ url('/houling/houling_home') }}");
      }
    }
    ...
    $(".js-car-id").select2({
    ajax: {
      url: "{{ url('/houling/houling_home/search_car_houling') }}",
      dataType: 'json',
      delay: 250,
    ...
    ajax: {
      url: "{{ url('/houling/houling_home/search_employee') }}",
      dataType: 'json',
      delay: 250,
      data: function (params) {
        return {
    ...
    $(".js-route-id").select2({
    ajax: {
      url: "{{ url('/houling/houling_home/search_route') }}",
      dataType: 'json',
      delay: 250,
      data: function (params) {
        return {
    ...
    ajax: {
      url: "{{ url('/houling/houling_home/search_tonase') }}",
      dataType: 'json',
      delay: 250,
      data: function (params) {
        return {
