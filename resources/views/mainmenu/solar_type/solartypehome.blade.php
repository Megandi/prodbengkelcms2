     ...
     <nav class="navbar navbar-static-top">
      ...
      <section class="content">
          <div class="row">
            ...
            {{-- search --}}
            <form class="form" action="{{ url('/mainmenu/solar_type_home/getrange') }}" method="post">
              {{-- set token --}}
              {{ csrf_field() }}
              ...
              <div class="box">
                ...
                <div class="box-footer">
                  <div class="pull-right">
                    <a style="width : 90px;" href="{{url('mainmenu/solar_type_home')}}" type="button" onclick="return confirm('Are you sure want to reset?')" class="btn btn-warning">Reset</a>
            ...
            {{-- tables --}}
              <div class="box box-primary">
                ...
                @if(DB::table('level_akses')->where('id_menu', '2')->where('id_level',auth::user()->level_id)->first()->c==1)
                  <a href="{{ url('/mainmenu/solar_type_home/add') }}" type="button" class="btn btn-primary">Add New Solar Type</a><br><br>
                  @endif
                ...
                @if(DB::table('level_akses')->where('id_menu', '2')->where('id_level',auth::user()->level_id)->first()->u==1)
                        <td><a style="width:90px;" href="{{ url('/mainmenu/solar_type_home/edit/'.$item->id) }}" type="button" class="btn btn-info">Edit</a></td>
                        @endif

                @if(DB::table('level_akses')->where('id_menu', '2')->where('id_level',auth::user()->level_id)->first()->d==1)
                        <td><a style="width:90px;" href="{{ url('/mainmenu/solar_type_home/delete/'.$item->id) }}" type="button" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this data ?')">Delete</a></td>
                        @endif
                      </tr>
                      @endforeach
                ...
                @if(DB::table('level_akses')->where('id_menu', '1')->where('id_level',auth::user()->level_id)->first()->e==1)
                  <div class="box-footer">
                     <form class="form" action="{{ url('/mainmenu/solar_type_home/export') }}" method="POST">
